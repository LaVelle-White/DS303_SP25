...
-   Which model seems to be the best fit according to the Adjusted R^2^ value?
    -   Model 3 had the best Adjusted R² value, which means it explained the most variance in the outcome (medv) after adjusting for the number of predictors. So in reguards of fit, it performed the strongest.
...
-   Which model has the lowest AIC?
    ...
    -   Model 3 had the lowest AIC at 1878.12. A lower AIC means better model fit while considering complexity, so this supports Model 3 as the best choice.

-   Are the improvements in fit (from the `anova` output) statistically significant?

    -   Yes. Both ANOVA tests showed highly significant p-values. This tells me that each time I added more predictors, the model got statistically better.
...
-   Which model performed best on the validation set?
    ...
    -   Model 3 had the lowest validation RMSE at 5.884, which means it made the most accurate predictions on new data compared to the other two models.

-   Does that match what AIC/ANOVA suggested?

    -   Yes, Model 3 had the lowest AIC and the most statistically significant improvement according to ANOVA. So all three methods agree that Model 3 is the strongest.
...
-   Is the test RMSE close to the validation RMSE?
    ...
    -   Yes. The test RMSE was **3.706**, and the validation RMSE was **5.884**. They're close enough to show the model is stable and can generalize well to new data.

-   What does that say about how well the model generalizes?

    -   Because the test RMSE is similar to the validation RMSE, it tells me that the model generalizes well and is likely to perform accurately on other unseen datasets.
...
-   The training RMSE was 4.27, and the validation/test RMSEs were 5.88 and 3.71. There's not a huge gap — so no clear sign of overfitting. The model seems well-balanced.
