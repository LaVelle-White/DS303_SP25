...
```{r}
poly.area <- with(P103, vowelMeansPolygonArea(f1, f2, ipa, poly.order = c("i", 
...
```{r}
par(mfrow = c(2, 2))
with(P103, plotVowels(f1, f2, var.sty.by = ipa, var.col.by = ipa, pretty = TRUE))
```

Add the IPA symbols to the visualization.

```{r}
with(P103, plotVowels(f1, f2, ipa, plot.tokens = TRUE, pch.tokens = ipa, 
...
```{r}
with(P103, plotVowels(f1, f2, ipa, group = ipa, plot.tokens = FALSE, plot.means = TRUE, 
