...
```{r}
m6 <- lm(wt ~ mpg + hp + qsec, data = mtcars)
summary(m6)
...
```{r}
m.interaction <- lm(mpg ~ wt * hp, data = mtcars)
summary(m.interaction)

anova(m2, m.interaction)

```

Final verdict is m.best is the optimal model has the lowest AIC and all of the predictors are significant.
