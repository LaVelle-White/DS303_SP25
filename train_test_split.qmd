...
#Remaining data for validation and test (40%)
temp_data <- mtcars[-train_index, ]

#Split remaining data into validation (20%) and test (20%) sets
val_index <- ceateDataPartition(temp_data$mpg, p = 0.5, list = FALSE)
...
```{r}
# Compare AIC values
aic_simple_model <- AIC(model_simple)
aic_complex_model <- AIC(model_complex)
...
anova_result <- anova(model_simple, model_complex)
...
#Calculate RMSE for model 1 on validation set
rmse_model_1_val <- rsme(val_data$mpg, predict(model_simple, val_data))

# Caluate RSME for Model 2 on validation set
rsme_model_2_val <- rsme(val_data$mpg, predict(model_complex, val_data))
...
```{r}
Revenue  <- read_excel(file.choose(), sheet = "Revenue")
head(Revenue)
```
